...
def execute(filters):
    ...
    data = frappe.db.sql(f"""
        SELECT
            ...
            {select_fields},
            cgst.tax_amount * COALESCE({filters.get('reversal_percentage')}, pi.reversal_percentage) / 100 AS rcgst,
            sgst.tax_amount * COALESCE({filters.get('reversal_percentage')}, pi.reversal_percentage) / 100 AS rsgst,
            igst.tax_amount * COALESCE({filters.get('reversal_percentage')}, pi.reversal_percentage) / 100 AS rigst,
            pi.state
        FROM
...
def get_conditions(filters):
    ...
    if filters.get('posting_date') and filters.get('posting_date')!="":
        conditions += f' AND pi.posting_date = "{filters.get("posting_date")}"'
       
    if filters.get('reversal_percentage'):
        select_fields = f", {filters.get('reversal_percentage')} AS `reversal_percentage`"
    else:
        select_fields = ", pi.reversal_percentage AS `reversal_percentage`"

    if filters.get('present_date') and filters.get('posting_date')!="":
        conditions += f' AND pi.posting_date = "{filters.get("posting_date")}"'
